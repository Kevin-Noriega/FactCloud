...
export const useFactura = () => {
  ...
  useEffect(() => {
    ...
    const cargarDatos = async () => {
      try {
        ...
        setClientes(
          Array.isArray(resClientes.data?.data) ? resClientes.data.data : [],
        );
  ...
  const agregarFormaPago = () =>
    setFormasPago((p) => [...p, { metodo: "", valor: "" }]);
  const eliminarFormaPago = (i) =>
    setFormasPago((p) => p.filter((_, x) => x !== i));
  ...
  const agregarProductoLocal = (p) => {
    ...
    setProductosSeleccionados((prev) => [
      ...prev,
      {
        ...
        impuestoCargo: "IVA_19",
        impuestoRetencion: "",
        tarifaIVA: p.tarifaIVA ?? 0,
        tarifaINC: p.tarifaINC ?? 0,
        tarifaRetencion: 0,
      },
    ]);
  };
