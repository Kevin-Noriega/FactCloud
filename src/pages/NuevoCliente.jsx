...
import {
  PlusCircle, Trash, ChevronDown, ChevronUp, QuestionCircle,
    ...
    <div className="line-field">
      {label && (
        <label className="line-label" style={{ color: labelColor }}>
          ...
          {label}{tooltip && <Tooltip texto={tooltip} />}
        </label>
      )}
...
export default function NuevoCliente() {
  ...
  const { cliente, guardando, handleChange, handleSubmit } = useCliente({
    ...
    open:            true,
    onSuccess:       (_, msg) => { alert(msg); navigate(-1); },
    onClose:         ()       => navigate(-1),
  });
  ...
  const [contactos,         setContactos]     = useState([{ nombre: "", apellido: "", correo: "", cargo: "", indicativo: "", telefono: "" }]);
  ...
  const handleGuardar = () => {
    ...
    handleSubmit({
      ...
      tipoRegimenIva:       contactoFact.tipoRegimenIva,
    ...
    <div className="crear-cliente-wrap">
      ...
      <div className="cc-header">
        ...
        <div className="cc-header-actions">
          <button className="btn-cancelar" onClick={() => navigate(-1)} disabled={guardando}>Cancelar</button>
        ...
        <div className="cc-card cc-datos-basicos">
          ...
          <div className="cc-basicos-grid">
            ...
            <div className="cc-col-left">
              <LineSelect label="Tipo" color="green"
                name="tipoPersona" value={cliente.tipoPersona}
              ...
              <LineSelect label="Tipo de identificación" required color="red"
                name="tipoIdentificacion" value={cliente.tipoIdentificacion}
                onChange={handleChange}>
              ...
              <div className="cc-id-row">
                ...
                <LineInput label="Dv" maxLength={1}
                  name="digitoVerificacion" value={cliente.digitoVerificacion}
                  onChange={handleChange} style={{ width: "56px" }} />
              </div>
              ...
              <div className="mt-2">
                <button className="btn-link-green" type="button">
                  Autocompletar datos <Tooltip texto="Busca los datos del tercero usando su identificación en el RUT de la DIAN." />
                </button>
              </div>
            ...
            <div className="cc-col-right">
              <LineInput label="Nombres" required
              ...
              <LineInput label="Apellidos"
                name="apellido" value={cliente.apellido} onChange={handleChange} />
              <LineInput label="Nombre comercial"
                name="nombreComercial" value={cliente.nombreComercial} onChange={handleChange} />
              <LineInput label="Ciudad" color="green"
                name="ciudad" value={cliente.ciudad} onChange={handleChange} />
              <LineInput label="Dirección"
                name="direccion" value={cliente.direccion} onChange={handleChange} />
            </div>
          </div>
        </div>
        ...
        <div className="cc-card cc-facturacion">
          <div className="cc-fact-grid">
            <div>
              ...
              <LineSelect label="Tipo de régimen IVA"
              ...
              <div className="cc-tel-row mt-2">
                <div style={{ width: "80px" }}>
                  <LineInput label="Indicativo"
                    value={contactoFact.indicativoFact}
                    onChange={(e) => setCF("indicativoFact", e.target.value)} />
                </div>
                <div style={{ flex: 1 }}>
                  <LineInput label="# de Teléfono"
                    name="telefono" value={cliente.telefono} onChange={handleChange} />
                </div>
              </div>
              <LineInput label="Código postal"
        ...
        {contactosOpen && (
          <div className="cc-contactos-body">
            {contactos.map((c, i) => (
              <div key={i} className="cc-contacto-row mb-3">
                ...
                <div className="flex-fill">
                  <LineInput label="* Nombre" required value={c.nombre}
                    onChange={(e) => updContact(i, "nombre", e.target.value)} />
                </div>
                <div className="flex-fill">
                  <LineInput label="Apellido" value={c.apellido}
                    onChange={(e) => updContact(i, "apellido", e.target.value)} />
                </div>
                <div className="flex-fill">
                  <LineInput label="Correo electrónico" type="email" value={c.correo}
                    onChange={(e) => updContact(i, "correo", e.target.value)} />
                </div>
                <div className="flex-fill">
                  <LineInput label="Cargo" value={c.cargo}
                    onChange={(e) => updContact(i, "cargo", e.target.value)} />
                </div>
                <div style={{ width: "80px" }}>
                  <LineInput label="Indicativo" value={c.indicativo}
                    onChange={(e) => updContact(i, "indicativo", e.target.value)} />
                </div>
                <div className="flex-fill">
                  <LineInput label="# de Teléfono" value={c.telefono}
            ...
            <button className="btn-link-green"
              onClick={() => setContactos([...contactos,
