...
function TooltipInfo({ texto }) {
  ...
  return (
    <>
      <i className="doc-info-icon" data-tooltip-id={id} data-tooltip-place="bottom">i</i>
      <Tooltip id={id} opacity={1} style={{ maxWidth: 260, fontSize: "0.8rem" }}>
        {texto}
      </Tooltip>
    </>
  );
...
const NoOptionsCliente  = ({ inputValue, onCrear }) => (
  <div>
    ...
    <div className="doc-dropdown-item doc-dropdown-crear" onMouseDown={e => { e.preventDefault(); onCrear(inputValue); }}>
      <PlusCircle size={13} /> Crear cliente "{inputValue}"
    </div>
  </div>
);
...
const NoOptionsProducto = ({ inputValue, onCrear }) => (
  <div>
    {inputValue && <div className="doc-select-empty">No se encontró "{inputValue}"</div>}
    <div className="doc-dropdown-item doc-dropdown-crear" onMouseDown={e => { e.preventDefault(); onCrear(inputValue); }}>
      <PlusCircle size={13} /> Crear producto "{inputValue}"
    </div>
  </div>
);
  ...
  return (
    <div className="doc-container">
      ...
      {facturasUsadas.limite > 0 && (
        <p className="doc-comprobantes">
          Tus facturas usadas:{" "}
          <span className={`doc-comprobantes-count ${facturasUsadas.usadas >= facturasUsadas.limite ? "danger" : ""}`}>
            {facturasUsadas.usadas} de {facturasUsadas.limite}
          </span>
        </p>
      )}
      ...
      <form onSubmit={e => { setSubmitIntentado(true); handleSubmit(e); }}>
        ...
        <div className="doc-header-grid" style={{ marginTop: 12 }}>
          <div className="doc-col">
            ...
            <div className="doc-field">
              ...
              <div style={{ minWidth: 0 }}>
                <Select
                  ...
                  options={clientes.map(c => ({ value: c.id, label: `${c.nombre} ${c.apellido} — ${c.numeroIdentificacion}` }))}
                  value={clientes.map(c => ({ value: c.id, label: `${c.nombre} ${c.apellido} — ${c.numeroIdentificacion}` })).find(o => o.value === factura.clienteId) ?? null}
                  onChange={opt => setFactura(p => ({ ...p, clienteId: opt?.value ?? "", contactoId: "" }))}
                  onBlur={() => marcar("clienteId")}
                  noOptionsMessage={({ inputValue }) => <NoOptionsCliente inputValue={inputValue} onCrear={handleCrearCli} />}
                  isClearable
            ...
            <div className="doc-field">
              <label className="doc-label">
                Contacto
                <TooltipInfo texto="Persona de contacto del cliente." />
              </label>
              <Select
                styles={mkSelectStyles()}
                menuPortalTarget={document.body}
                options={contactos.map(c => ({ value: c.id, label: `${c.nombre}${c.cargo ? ` — ${c.cargo}` : ""}` }))}
                value={contactos.map(c => ({ value: c.id, label: `${c.nombre}${c.cargo ? ` — ${c.cargo}` : ""}` })).find(o => o.value === factura.contactoId) ?? null}
                onChange={opt => setFactura(p => ({ ...p, contactoId: opt?.value ?? "" }))}
                noOptionsMessage={() => <NoOptionsContacto onAbrirModal={() => setMostrarModalContacto(true)} />}
                isClearable
          ...
          <div className="doc-col">
            ...
            <div className="doc-field">
              ...
              <div className="doc-comprobante-inputs">
                <input type="text" className="form-control form-control-sm doc-prefijo text-center fw-bold"
                  value={factura.prefijo} readOnly />
                <input type="text" className="form-control form-control-sm text-center text-primary fw-bold"
                  value={factura.numeroFactura} placeholder="Auto" readOnly />
              </div>
            </div>
            ...
            <div className="doc-field">
              ...
              <div className="input-group input-group-sm">
                <input ref={barcodeInputRef} type="text" className="form-control"
                  value={codigoBarras} onChange={e => setCodigoBarras(e.target.value)}
                  onKeyDown={handleBarcodeInput} placeholder="Código de barras..." autoComplete="off" />
            ...
            <table className="doc-tabla">
              ...
              <tbody>
                {productosSeleccionados.length === 0 ? (
                  <tr><td colSpan="9" className="doc-tabla-empty">
                    ...
                    return (
                      <tr key={idx}>
                        ...
                        <td>
                          <Select
                            menuPortalTarget={document.body}
                            styles={{ ...mkSelectStyles(), control: (b, s) => ({ ...mkSelectStyles().control(b, s), minWidth: 200 }) }}
                            options={productos.map(p => ({ value: p.id, label: `${p.nombre} — $${p.precioUnitario.toLocaleString("es-CO")}` }))}
                            value={productos.map(p => ({ value: p.id, label: `${p.nombre} — $${p.precioUnitario.toLocaleString("es-CO")}` })).find(o => o.value === item.productoId) ?? null}
                            onChange={opt => actualizarProducto(idx, "productoId", opt?.value ?? "")}
                            noOptionsMessage={({ inputValue }) => <NoOptionsProducto inputValue={inputValue} onCrear={handleCrearPro} />}
                            isClearable
                        ...
                        <td>
                          <input type="number" className="form-control form-control-sm text-end doc-num-input"
                            value={item.precioUnitario} step="0.01"
                        ...
                        <td>
                          <select className="form-select form-select-sm doc-select-item"
                            value={item.tarifaIVA} onChange={e => actualizarProducto(idx, "tarifaIVA", e.target.value)}>
                        ...
                        <td>
                          <select className="form-select form-select-sm doc-select-item"
                            value={item.tarifaINC} onChange={e => actualizarProducto(idx, "tarifaINC", e.target.value)}>
                            <option value="0">0%</option>
        ...
        <div className="doc-pago-totales">
            ...
            {formasPago.map((fp, idx) => (
              <div key={idx} className="doc-pago-fila">
                <select className="form-select form-select-sm doc-pago-select"
                  value={fp.metodo} onChange={e => actualizarFormaPago(idx, "metodo", e.target.value)}>
                  <option value="">Selecciona forma de pago</option>
                  {OPCIONES_FORMA_PAGO.map(op => <option key={op.value} value={op.value}>{op.label}</option>)}
                </select>
          ...
          <div className="doc-totales">
            <div className="doc-totales-fila"><span>Total Bruto:</span><span>{fmt(totales.subtotal)}</span></div>
            ...
            {totales.totalIVA > 0 && (
              <div className="doc-totales-fila"><span>IVA:</span><span>{fmt(totales.totalIVA)}</span></div>
            )}
            {totales.totalINC > 0 && (
              <div className="doc-totales-fila"><span>INC:</span><span>{fmt(totales.totalINC)}</span></div>
            )}
            <div className="doc-totales-fila">
              <span>RetelCA:</span>
              <div className="doc-retelca-row">
                <select className="form-select form-select-sm doc-retelca-select"
                  value={retelCA.tipo} onChange={e => setRetelCA({ tipo: e.target.value, valor: 0 })}>
                  <option value=""></option>
        ...
        <div className="doc-observaciones">
          ...
          <label className="doc-adjuntar">
            <input type="file" className="d-none"
              onChange={e => { const f = e.target.files[0]; if (f) setFactura(p => ({ ...p, archivo: f })); }} />
            <Paperclip size={14} /> Adjuntar archivo
            {factura.archivo && <span className="doc-archivo-nombre">{factura.archivo.name}</span>}
          </label>
        </div>

        {/* ── Footer sticky ── */}
        <div className="doc-footer">
          <button type="button" className="doc-btn-cancelar" onClick={() => navigate("/ventas")}>Cancelar</button>
          <button type="submit" className="doc-btn-guardar-enviar">Guardar y crear factura</button>
        </div>
      </form>

      {/* Modales */}
      {mostrarModalContacto && (
        <ModalCrearContacto clienteId={factura.clienteId}
          onClose={() => setMostrarModalContacto(false)}
          onSuccess={c => { agregarContactoLocal(c); setMostrarModalContacto(false); }} />
      )}
      {mostrarModalCliente && (
        <ModalCrearCliente open nombreSugerido={nombreSugeridoCliente}
          onClose={() => setMostrarModalCliente(false)}
          onSuccess={c => { agregarClienteLocal(c); setMostrarModalCliente(false); }} />
      )}
      {mostrarModalProducto && (
        <ModalCrearProducto open nombreSugerido={nombreSugeridoProducto}
