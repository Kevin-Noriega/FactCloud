...
export default function PlanCard({ plan, onCheckout }) {
  ...
  const hasDiscount = plan.hasDiscount && plan.discountPercentage;
  const monthlyPriceFormatted = plan.monthlyPrice.toLocaleString("es-CO");
  const annualPriceFormatted = plan.annualPrice.toLocaleString("es-CO");
  const originalAnnualFormatted = plan.originalAnnualPrice
