          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="row mb-3">
              <div className="col-md-4">
                ...
                <select name="tipoPersona" className="form-select"
                  value={cliente.tipoPersona} onChange={handleChange} required>
              ...
              <div className="col-md-4">
                ...
                <Select
                  options={tipoIdentificacion.map((ti) => ({ value: ti.nombre, label: `${ti.codigo} - ${ti.nombre}` }))}
                  value={tipoIdentificacion.map((ti) => ({ value: ti.nombre, label: `${ti.codigo} - ${ti.nombre}` })).find((opt) => opt.value === cliente.tipoIdentificacion) ?? null}
                  onChange={(opt) => handleSelectChange("tipoIdentificacion", opt ? opt.value : "")}
                  isClearable placeholder="Seleccionar tipo"
                />
              </div>
              <div className="col-md-4">
                <label className="form-label">Responsabilidad tributaria *</label>
                <Select
                  options={regimenTributarioDIAN.map((rt) => ({ value: rt.descripcion, label: `${rt.codigo} - ${rt.descripcion}` }))}
                  value={regimenTributarioDIAN.map((rt) => ({ value: rt.descripcion, label: `${rt.codigo} - ${rt.descripcion}` })).find((opt) => opt.value === cliente.regimenTributario) ?? null}
                  onChange={(opt) => handleSelectChange("regimenTributario", opt ? opt.value : "")}
                  isClearable placeholder="Seleccionar régimen"
                />
              </div>
            </div>
            ...
            <div className="row mb-3">
              <div className="col-md-4">
                ...
                <input type="text" name="numeroIdentificacion" className="form-control"
                  value={cliente.numeroIdentificacion} onChange={handleChange} required />
              </div>
              <div className="col-md-2">
                <label className="form-label">Dígito Verificación</label>
                <input type="number" name="digitoVerificacion" className="form-control"
                  value={cliente.digitoVerificacion} onChange={handleChange} min="0" max="9" />
              </div>
              <div className="col-md-6">
                <label className="form-label">Tipo de responsabilidad *</label>
                <Select
                  options={regimenFiscalDIAN.map((rf) => ({ value: rf.descripcion, label: `${rf.codigo} - ${rf.descripcion}` }))}
                  value={regimenFiscalDIAN.map((rf) => ({ value: rf.descripcion, label: `${rf.codigo} - ${rf.descripcion}` })).find((opt) => opt.value === cliente.regimenFiscal) ?? null}
            ...
            <div className="row mb-3">
              <div className="col-md-6">
                ...
                <input type="text" name="nombreComercial" className="form-control"
                  value={cliente.nombreComercial} onChange={handleChange} />
              </div>
              <div className="col-md-6">
                <label className="form-label">Actividad Económica CIIU</label>
                <Select
                  options={actividadesCIIU.map((act) => ({ value: act.codigo, label: `${act.codigo} - ${act.descripcion}` }))}
                  value={actividadesCIIU.map((act) => ({ value: act.codigo, label: `${act.codigo} - ${act.descripcion}` })).find((opt) => opt.value === cliente.actividadEconomicaCIIU) ?? null}
                  onChange={(opt) => handleSelectChange("actividadEconomicaCIIU", opt ? opt.value : "")}
                  isClearable placeholder="Seleccionar actividad"
                />
              </div>
            </div>
            ...
            <div className="row mb-3">
              ...
              <div className="col-md-6">
                ...
                <input type="text" name="apellido" className="form-control"
                  value={cliente.apellido} onChange={handleChange} required />
              </div>
            </div>
            ...
            <div className="row mb-3">
              <div className="col-md-6">
                ...
                <input type="email" name="correo" className="form-control"
                  value={cliente.correo} onChange={handleChange} required />
              </div>
              <div className="col-md-6">
            ...
            <div className="row mb-3">
              <div className="col-md-6">
                ...
                <Select
                  ...
                  value={departamentosOptions.find((opt) => String(opt.departamentoCodigo) === String(cliente.departamentoCodigo)) ?? null}
                  onChange={handleDepartamentoChange}
                  placeholder="Seleccionar departamento" isClearable
                />
              </div>
              <div className="col-md-6">
                ...
                <Select
                  ...
                  value={ciudadesOptionsPorDepartamento(cliente.departamento).find((opt) => String(opt.ciudadCodigo) === String(cliente.ciudadCodigo)) ?? null}
                  onChange={handleCiudadChange}
                  placeholder="Seleccionar ciudad" isClearable
            ...
            <div className="row mb-3">
              ...
              <div className="col-md-2">
                ...
                <input type="text" name="codigoPostal" className="form-control"
                  value={cliente.codigoPostal} onChange={handleChange} />
              </div>
            </div>
                ...
                <div className="col-md-3" key={name}>
                  <div className="form-check">
                    <input type="checkbox" name={name} className="form-check-input"
                      checked={cliente[name]} onChange={handleChange} />
              ...
              <div className="row mb-2 align-items-end" key={i}>
                <div className="col-md-2">
                  <input className="form-control" placeholder="* Nombre"
                    value={c.nombre} onChange={(e) => handleContactoChange(i, "nombre", e.target.value)} />
                </div>
                <div className="col-md-2">
                  <input className="form-control" placeholder="Apellido"
                    value={c.apellido} onChange={(e) => handleContactoChange(i, "apellido", e.target.value)} />
                </div>
                <div className="col-md-3">
                  <input className="form-control" placeholder="Correo electrónico"
                    value={c.correo} onChange={(e) => handleContactoChange(i, "correo", e.target.value)} />
                </div>
                <div className="col-md-2">
                  <input className="form-control" placeholder="Cargo"
                    value={c.cargo} onChange={(e) => handleContactoChange(i, "cargo", e.target.value)} />
                </div>
                <div className="col-md-1">
                  <input className="form-control" placeholder="Indicativo"
                    value={c.indicativo} onChange={(e) => handleContactoChange(i, "indicativo", e.target.value)} />
                </div>
                <div className="col-md-1">
                  <input className="form-control" placeholder="# Tel"
                    value={c.telefono} onChange={(e) => handleContactoChange(i, "telefono", e.target.value)} />
                </div>
                {i > 0 && (
                  <div className="col-md-1">
                    <button type="button" className="btn btn-link text-danger p-0"
                      onClick={() => eliminarContacto(i)}>Eliminar</button>
                  </div>
                )}
              </div>
            ))}
            <button type="button" className="btn btn-link text-success p-0 mt-1 mb-3"
            ...
            <div className="modal-cliente-footer">
              <button type="button" className="btn btn-secondary"
                onClick={handleClose} disabled={guardando}>
                Cancelar
              </button>
